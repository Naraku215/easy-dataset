...
async function splitFileByType({ projectPath, fileContent, fileName, projectId, fileId }) {
  ...
  } else if (splitType === 'advanced-markdown') {
    // 高级 Markdown 分块（原子块感知，表格/公式/列表保护）
    const { splitAdvancedMarkdown } = require('./split-markdown/advanced/index');
    const advancedConfig = {
      minLength: taskConfig.advancedTextSplitMinLength || 800,
      maxLength: taskConfig.advancedTextSplitMaxLength || 2000,
      overlap: taskConfig.chunkOverlap || 0,
      preserveHeadings: taskConfig.advancedPreserveHeadings !== false
    };
    const { chunks } = splitAdvancedMarkdown(fileContent, advancedConfig);
