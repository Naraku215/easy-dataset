...
const { chunkSections, mergeShortChunks, buildSectionText } = require('./chunker');
...
function splitAdvancedMarkdown(markdownText, config = {}) {
  ...
  // 3.5 统计语义段落长度分布（供前端参考设置切割参数）
  const sectionSizes = sections.map(s => buildSectionText(s).length).filter(len => len > 0);
  const sectionStats = {
    minSectionSize: sectionSizes.length > 0 ? Math.min(...sectionSizes) : 0,
    medianSectionSize: sectionSizes.length > 0 ? median(sectionSizes) : 0,
    maxSectionSize: sectionSizes.length > 0 ? Math.max(...sectionSizes) : 0
  };
  ...
  // 10. 统计信息
  const stats = { ...scoreAll(cleanChunks, mergedConfig), ...sectionStats };
